...
class AuthIntegrityService {
  ...
  void startPeriodicChecks({Duration interval = const Duration(hours: 24)}) {
    ...
    debugPrint(
        'AuthIntegrityService: Started periodic integrity checks, interval: ${interval.inHours}h');
  }
  ...
  Future<void> logAuthError({
    ...
    try {
      // 🚧 개발 중 Firestore 로깅 비활성화 (Permission denied 방지)
      debugPrint(
          'AuthIntegrityService: Auth error - $operation: $errorMessage');

      // 운영 환경에서만 Firestore 로깅 활성화
      if (kReleaseMode) {
        final logEntry = {
          'operation': operation,
        ...
        debugPrint('AuthIntegrityService: Logged auth error to Firestore');
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> performIntegrityCheck(
    ...
    try {
      ...
      await _firestore
          .collection('system_logs')
          .doc('integrity_checks')
          .collection('checks')
          .add(result);

      debugPrint(
          'AuthIntegrityService: Integrity check completed - ${result['autoFixed']} issues auto-fixed');
    } catch (e) {
  ...
  Future<void> _checkUserIntegrity(
      String uid, bool autoFix, Map<String, dynamic> result) async {
    try {
      ...
      try {
        ...
        if (authUser == null || authUser.uid != uid) {
          ...
          debugPrint(
              'AuthIntegrityService: User exists in Firestore but not in Firebase Auth - $uid');
          return;
        }
      } catch (e) {
        result['missingAuthUsers']++;
        debugPrint(
            'AuthIntegrityService: Error checking Firebase Auth user - $e');
        return;
      }
      ...
      if (!userDoc.exists) {
        ...
        debugPrint(
            'AuthIntegrityService: User exists in Firebase Auth but not in Firestore - $uid');
        ...
        if (autoFix) {
          ...
          result['autoFixed']++;
          debugPrint(
              'AuthIntegrityService: Auto-fixed - Created Firestore document for user - $uid');
        }
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> adminCheckAllUsers() async {
    final result = {
      'timestamp': DateTime.now().toIso8601String(),
      'message':
          'Full integrity check requires Admin SDK and should be run as a scheduled Cloud Function',
      'recommendation':
          'Implement a Cloud Function that uses the Firebase Admin SDK to list all users',
    };

    debugPrint(
        'AuthIntegrityService: Admin-level integrity check requires Cloud Functions with Admin SDK');
    return result;
  }
}
